<template>
    <div v-if="ad && !dismissed" :class="containerClasses">
        ...
        >
            ...
            />
            <!-- Hover overlay -->
            <div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition rounded-t-xl"></div>
        </a>
        <div class="bg-surface-light-card dark:bg-surface-dark-card px-2 py-1 flex items-center justify-between rounded-b-xl">
            <span class="text-[10px] text-slate-400 uppercase tracking-wider">Sponsored</span>
            <button
                @click="dismiss"
    ...
    <div v-else-if="!ad && showPlaceholder && !dismissed" :class="containerClasses">
        <div :class="['flex items-center justify-center bg-surface-light dark:bg-surface-dark rounded-xl border border-dashed border-surface-light-border dark:border-surface-dark-border text-slate-400 text-xs', placeholderHeightClass]">
...
const props = defineProps({
    ...
    placement: {
        type: String,
        default: 'sidebar',
        validator: v => ['sidebar', 'banner', 'between-rounds', 'leaderboard', 'header'].includes(v),
    },
...
const containerClasses = computed(() => {
    const base = 'rounded-xl overflow-hidden border border-surface-light-border dark:border-surface-dark-border';
    const placements = {
        sidebar: `${base}`,
        banner: `${base} w-full`,
